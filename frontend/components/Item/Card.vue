<template>
  <NuxtLink
    class="group card rounded-md border border-gray-300"
    :to="`/item/${item.id}`"
  >
    <div class="relative h-[200px]">
      <img
        v-if="imageUrl"
        class="h-[200px] bg-base-100 w-full object-contain rounded-t shadow-sm border-gray-300"
        :src="imageUrl"
      />
      <div class="absolute bottom-3 left-3">
        <NuxtLink
          v-if="item.location"
          class="text-sm hover:link badge badge-neutral shadow-md rounded-md"
    ...
    <div
      class="rounded-b p-4 pt-2 flex-grow col-span-4 flex flex-col gap-y-1 bg-base-100"
    >
      <div role="listitem" class="text-lg font-bold flex justify-between">
        <span>{{ item.name }}</span>
        <div class="tooltip" data-tip="Quantity">
          <span class="badge h-6 w-6 badge-primary badge-sm text-xs">
            x{{ item.quantity }}
          </span>
        </div>
      </div>
...
const imageUrl = computed(() => {
  ...
  return api.authURL(
    `/items/${props.item.id}/attachments/${props.item.imageId}`,
  )
})

...
watchDebounced(
  [page, pageSize, query, selectedLabels, selectedLocations],
  search,
...
<template>
  <BaseContainer class="mb-16">
    <div v-if="locations && labels">
      ...
      <div class="flex flex-wrap md:flex-nowrap gap-2 w-full py-2">
        ...
        <div class="dropdown">
          ...
          <div
            tabindex="0"
            class="dropdown-content mt-1 max-h-72 p-4 w-64 z-[1] overflow-auto shadow-lg bg-base-100 rounded-md -translate-x-24"
          >
        ...
        <div class="dropdown ml-auto dropdown-end">
          ...
          <div
            tabindex="0"
            class="dropdown-content mt-1 p-4 w-[325px] z-[1] text-sm overflow-auto shadow bg-base-100 rounded-md"
          >
            <p class="text-base">Search Tips</p>
            <ul class="mt-1 list-disc pl-6">
              <li>
                Location and label filters use the 'OR' operation. If more than
                one is selected only one will be required for a match.
              </li>
              <li>
                Searches prefixed with '#'' will query for a asset ID (example
                '#000-001')
              </li>
              <li>
                Field filters use the 'OR' operation. If more than one is
                selected only one will be required for a match.
              </li>
            </ul>
          </div>
        </div>
      ...
      <div v-if="fieldSelector" class="py-4 space-y-2">
        ...
        >
          <div class="form-control w-full max-w-xs">
            ...
            >
              <option v-for="(fv, _, i) in allFields" :key="i" :value="fv">
                {{ fv }}
              </option>
            </select>
          </div>
          <div class="form-control w-full max-w-xs">
            ...
            >
              <option v-for="v in fieldValuesCache[f[0]]" :key="v" :value="v">
                {{ v }}
              </option>
            </select>
          </div>
    ...
    <section class="mt-10">
      <BaseSectionHeader ref="itemsTitle">{{ $t('headers.items') }}</BaseSectionHeader>
      <p class="text-base font-medium flex items-center">
        {{ $t('table.results', [total]) }}
        <span class="text-base ml-auto">
          {{ $t('table.pageOfTotal', [page, totalPages]) }}
        </span>
      </p>

      <div
        ref="cardgrid"
        class="grid mt-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4"
      >
        <ItemCard v-for="item in items" :key="item.id" :item="item" />

        <div class="hidden first:inline text-xl">{{ $t('table.noItems') }}</div>
      </div>
      <div
        v-if="items.length > 0 && (hasNext || hasPrev)"
        class="mt-10 flex gap-2 flex-col items-center"
      >
        <div class="flex">
          <div class="btn-group">
            <button
              :disabled="!hasPrev"
              class="btn text-no-transform"
              @click="prev"
            >

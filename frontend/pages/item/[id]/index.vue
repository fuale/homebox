...
import type {
  ...
  Detail,
  Details,
} from "~~/components/global/DetailsSection/types"
...
const itemDetails = computed<Details>(() => {
  ...
  const ret: Details = [
    {
      name: i18n.t("item.quantity"),
      text: item.value?.quantity,
      slot: "quantity",
    },
    {
      name: i18n.t("item.serialNumber"),
      text: item.value?.serialNumber,
      copyable: true,
    },
    {
      name: i18n.t("item.modelNumber"),
      text: item.value?.modelNumber,
      copyable: true,
    },
    {
      name: i18n.t("item.manufacturer"),
      text: item.value?.manufacturer,
      copyable: true,
    },
    {
      name: i18n.t("item.insured"),
      text: item.value?.insured ? "Yes" : "No",
    },
    {
      name: i18n.t("item.notes"),
      type: "markdown",
...
<template>
  <BaseContainer v-if="item" class="pb-8">
    ...
    <dialog ref="refDialog" class="z-[999] fixed bg-transparent">
      <div ref="refDialogBody" class="relative">
        <div class="absolute right-0 -mt-3 -mr-3 sm:-mt-4 sm:-mr-4 space-x-1">
          <a
            class="btn btn-sm sm:btn-md btn-primary btn-circle"
            :href="dialoged.src"
            download
          >
    ...
    <section>
      <div class="bg-base-100 rounded p-3">
        <header class="mb-2">
          <div class="flex flex-wrap items-end gap-2">
            ...
            <div>
              <div
                v-if="fullpath && fullpath.length > 0"
      ...
      <div class="flex flex-wrap items-center justify-start mb-6 mt-3">
        <div class="join">
          <NuxtLink
            key="details"
            :to="`/item/${itemId}`"
          ...
          <NuxtLink
            key="maintenance"
            :to="`/item/${itemId}/maintenance`"
            class="btn join-item btn-sm"
            exact-active-class="btn-active"
          >
            {{ $t("button.tabMaintenance") }}
          </NuxtLink>
        </div>
    ...
    <section>
      <div class="space-y-6">
        <BaseCard>
          <div class="flex flex-wrap justify-between items-center p-4 gap-4">
            <label class="label cursor-pointer">
              <input
                v-model="preferences.showEmpty"
                type="checkbox"
                class="toggle toggle-primary"
              />
              <span class="label-text ml-4">{{ $t("button.showEmpty") }}</span>
            </label>
            <PageQRCode />
          </div>
        </BaseCard>
        <BaseCard v-if="!hasNested" collapsable>
          <template #title>{{ $t("button.tabDetails") }}</template>
        ...
        <template v-if="!hasNested">
          ...
          <BaseCard v-if="showAttachments" collapsable>
            ...
            <DetailsSection
              v-if="attachmentDetails.length > 0"
